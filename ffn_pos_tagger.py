...
def extract_columns_from_text(text, P, S):
    result = []
    ...
    for sent_id, sentence in enumerate(sentences, start=1):
        ...
        for token_id, token in enumerate(tokens, start=1):
            if token_id == 1:
                for i in range(P):
                    sent_tokens.append((i + 1, "<s>", "<s>"))
                sent_tokens.append((token_id + P, token, "<s>"))  # "<UNK>" for unknown POS tag
            elif token_id == len(tokens):
                sent_tokens.append((token_id + P, token, "<s>"))  # "<UNK>" for unknown POS tag
                for i in range(S):
                    sent_tokens.append((i + len(tokens) + P + 1, "</s>", "</s>"))
            else:
                sent_tokens.append((token_id + P, token, "<s>"))  # "<UNK>" for unknown POS tag
        result.append(sent_tokens)
    return result
...
def tag_sentence(sentence, model, vocabulary, P, S, tag_to_index):
    with torch.no_grad():
        model = model.eval()
        words = nltk.word_tokenize(sentence)
        words_idx_seq = []
        for i in range(len(words)):
            words_idx_seq.extend(create_list_with_neighbours(words, i, P, S))

        words_idx_seq = torch.tensor([vocabulary[word] for word in words_idx_seq]).to(device)
        pred = model(words_idx_seq)
        _, predicted_labels = torch.max(pred, 1)

        tagged_sentence = []
        for i in range(P, len(words) - S):
            tag_index = predicted_labels[i - P].item()
            tag = list(tag_to_index.keys())[list(tag_to_index.values()).index(tag_index)]
            tagged_sentence.append(f"{words[i]} {tag}")
...
if __name__ == "__main__":
    ...
    model_filename = f"ffn_model_{EMBEDDING_DIM}_{BATCH_SIZE}_{HIDDEN_DIM}_{NUM_LAYERS}_{CONTEXT_WINDOW}_{LEARNING_RATE}_{NUM_EPOCHS}.pt"
    ...
    # Example usage of the tag_sentence function
    data_input = []
    input_sentence = input("Enter a sentence: ")
    input_data = extract_columns_from_text(input_sentence, P, S)
    # print(input_data)
    ...
    for sentence in input_data:
        ...
        for token in sentence:
            words.append(token[1])
            tagIndexes.append(tag_to_index[token[2]])
        data_input.append((words, tagIndexes))

    # print(data_input)

    dataset_input = MyDataset(data_input, P, S, vocabulary=dataset_train.vocabulary)
    dataloader_input = DataLoader(dataset=dataset_input, batch_size=1, shuffle=False)

    with torch.no_grad():
        for batch_num, (words_idx_seq, words_idx_seq_labels) in enumerate(dataloader_input):
            (words_idx_seq, words_idx_seq_labels) = (words_idx_seq.to(device), words_idx_seq_labels.to(device))
            pred = model(words_idx_seq)
            _, predicted_labels = torch.max(pred, 1)
            # print(words_idx_seq[0, P].item(), predicted_labels.item())
            tag_index = predicted_labels.item()
            tag = None
            for t, idx in tag_to_index.items():
                if idx == tag_index:
                    tag = t
                    break
            print(input_data[0][P + batch_num][1], tag)
