...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        for batch_num, (words_idx_seq, words_idx_seq_labels) in enumerate(dataloader_dev):
            ...
            true_labels.extend(words_idx_seq_labels.cpu().numpy())
    ...
    sns.heatmap(conf_matrix, annot=True, cmap="Blues", fmt="d", xticklabels=tag_to_index.keys(), yticklabels=tag_to_index.keys())
    plt.xlabel('Predicted')
    plt.ylabel('True')
    plt.title('Confusion Matrix')
    # Save the figure
    plt.savefig('ffn_confusion_matrix.png')
    plt.show()
